...
parser.add_argument("--batch_size",
                    ...
                    default=32,  # Reduced for M3 Ultra memory
                    type=int)
...
parser.add_argument("--img_size", 
                    ...
                    default=128,  # Reduced for M3 Ultra
                    type=int)
...
class Sampler:
    ...
    def to(self, device):
        ...
        self.alpha_cumulative_prod = self.alpha_cumulative_prod.to(device)
        return self
    ...
    def add_noise(self, inputs, timesteps):
        ...
        # Index with CPU timesteps, then move to device
        alpha_cumulative_prod_timesteps = self.alpha_cumulative_prod[timesteps.cpu()].to(device)
    ...
    def remove_noise(self, input, timestep, predicted_noise):
        ...
        # Move timesteps to CPU for indexing
        timestep_cpu = timestep.cpu()

        ### Create a mask (if timestep == 0 sigma_z will also be 0 so we need to save this for later ###
        greater_than_0_mask = (timestep >= 1).int().to(device)

        
        ### Compute Sigma (b_t * (1 - cumulative_a_(t-1)) / (1 - cumulative_a)) * noise ###
        alpha_cumulative_t = self.alpha_cumulative_prod[timestep_cpu].to(device)
        alpha_cumulative_prod_t_prev = self.alpha_cumulative_prod[timestep_cpu - 1].to(device)
        beta_t = self.beta_schedule[timestep_cpu].to(device)
        ...
        ### Compute 1 / sqrt(a_t) ###
        reciprocal_root_a_t = (self.alpha[timestep_cpu]**-0.5).to(device)
...
class SelfAttention(nn.Module):
  ...
  def __init__(self,
               in_channels,
               num_heads=4,  # Reduced for MPS
               attn_p=0,
               proj_p=0,
               fused_attn=False):  # Disabled for MPS compatibility
  ...
  def forward(self, x):
    ...
    # Use manual attention for MPS compatibility
...
class TransformerBlock(nn.Module):
    def __init__(self,
                 in_channels,
                 fused_attention=False,  # Disabled for MPS
                 num_heads=4, 
                 mlp_ratio=2,
                 proj_p=0,
                 attn_p=0,
...
class UpSampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.upsample = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='nearest'),  # Explicit mode for MPS
...
def sample_plot_image(step_idx, 
    ...
    images = torch.randn((num_gens, num_channels, image_size, image_size), device=device)
    num_images_per_gen = (total_timesteps // plot_freq)

    images_to_vis = [[] for _ in range(num_gens)]
    for t in tqdm(np.arange(total_timesteps)[::-1], desc="Generating images", leave=False):
        ts = torch.full((num_gens, ), t, device=device)
        noise_pred = model(images, ts)
        images = sampler.remove_noise(images, ts, noise_pred)
        if t % plot_freq == 0:
            for idx, image in enumerate(images):
                images_to_vis[idx].append(tensor_to_image(image))

    images_to_vis = list(itertools.chain(*images_to_vis))

    fig, axes = plt.subplots(nrows=num_gens, ncols=num_images_per_gen, figsize=(num_images_per_gen*2, num_gens*2))
    plt.tight_layout()
    if num_gens == 1:
    ...
    plt.savefig(os.path.join(path_to_generated_dir, f"step_{step_idx}.png"), dpi=100, bbox_inches='tight')
    plt.close()
...
class CelebADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply transforms directly here to avoid pickling issues
...
def main():
    ...
    os.makedirs(args.generated_directory, exist_ok=True)
    ...
    if args.use_wandb:
        try:
            import wandb
            wandb.init(project="diffusion-mac-m3", name=args.experiment_name, config=vars(args))
            use_wandb = True
        except ImportError:
    ...
    dataset = CelebADataset(args.path_to_data, img_size=args.img_size)
    print(f"Dataset size: {len(dataset)} images")
    
    # Optimize DataLoader for Mac
    ...
    if (not args.bias_weight_decay) or (not args.norm_weight_decay):
        print("Disabling Weight Decay on Some Parameters")
        weight_decay_params = []
        no_weight_decay_params = []
        for name, param in model.named_parameters():
            if param.requires_grad:
                ### Dont have Weight decay on any bias parameter ###
                if "bias" in name and not args.bias_weight_decay:
                    no_weight_decay_params.append(param)
                ### Dont have Weight Decay on any Norm scales params ###
    ...
    if args.resume_from_checkpoint is not None:
        ...
        checkpoint = torch.load(os.path.join(path_to_checkpoint, "model.pt"), map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
        completed_steps = checkpoint['step']
        print(f"Resuming from Iteration: {completed_steps}")

    ### DEFINE DDPM SAMPLER ###
    ddpm_sampler = Sampler(total_timesteps=args.num_diffusion_timesteps)
    ddpm_sampler = ddpm_sampler.to(device)  # Move sampler tensors to device
    print(f"✓ DDPM Sampler initialized with {args.num_diffusion_timesteps} timesteps")
    ...
    progress_bar = tqdm(range(completed_steps, args.num_training_steps), desc="Training")
    accumulated_loss = 0
    train = True
    ...
    while train:
        for images in trainloader:
            ...
            for _ in range(args.gradient_accumulation_steps):
                ...
                ### Random Sample T ###
                timesteps = torch.randint(0, args.num_diffusion_timesteps, (batch_size,), device=device)
                ...
                ### Compute Error ###
                loss = loss_fn(noise_pred, noise) / args.gradient_accumulation_steps
            ...
            if completed_steps % 100 == 0:
                print(f"Step {completed_steps}, Loss: {accumulated_loss:.4f}, LR: {scheduler.get_last_lr()[0]:.6f}")
            ...
            if completed_steps % args.evaluation_interval == 0:
                ...
                torch.save({
                    ...
                    'loss': accumulated_loss,
                }, os.path.join(path_to_checkpoint, "model.pt"))

                ### Delete Old Checkpoints ###
                all_checkpoints = [d for d in os.listdir(path_to_experiment) if d.startswith("checkpoint_")]
                ...
                with torch.no_grad():
                    ...
                    if use_wandb:
                        wandb.log({"generated_images": wandb.Image(fig)}, step=completed_steps)
                model.train()
